class SyncedFileSystem extends FileSystem {
  ...
  constructor({contents = null, doSyncImmediately = false} = {}) {
    ...
    if (doSyncImmediately) {
      ...
      var storage = {};
      keys.forEach((key) => { storage["FILE_" + key] = this.allFiles[key]; });
  ...
  set(path, newItem) {
    ...
    var newName = path.split("/").slice(0, -1).join("/") + "/" + newItem.name;
  ...
  delete(path) {
    ...
    keys.forEach((key) => { chrome.storage.sync.remove("FILE_" + key); });
  }
  ...
  static logStorage() { chrome.storage.sync.get(null, console.log); }

  static clear() { chrome.storage.sync.remove("fileSystem"); }
